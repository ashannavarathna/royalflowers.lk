    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            if (request.getParameter("pcode") != null && !request.getParameter("pcode").isEmpty()) {
                ...
                Criteria cry = sex.createCriteria(pojo.Product.class);
                cry.add(Restrictions.eq("productCode", pcode));
                
                pojo.Product product = (pojo.Product) cry.uniqueResult();
                if (product != null) {
                    pojo.PStatus ps = (pojo.PStatus) sex.load(pojo.PStatus.class, 2);
                    product.setPStatus(ps);
                    
                    sex.save(product);
                    out.write("Remove success");
